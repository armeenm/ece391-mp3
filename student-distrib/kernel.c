...
void entry(unsigned long magic, unsigned long addr) {
  ...
  if (CHECK_FLAG(mbi->flags, 5)) {
    ...
    printf("elf_sec: num = %u, size = 0x%#x, addr = 0x%#x, shndx = 0x%#x\n", (unsigned)elf_sec->num,
           (unsigned)elf_sec->size, (unsigned)elf_sec->addr, (unsigned)elf_sec->shndx);
  }
  ...
  if (CHECK_FLAG(mbi->flags, 6)) {
    ...
    for (mmap = (memory_map_t*)mbi->mmap_addr;
      ...
      printf("    size = 0x%x, base_addr = 0x%#x%#x\n    type = 0x%x,  length    = 0x%#x%#x\n",
             (unsigned)mmap->size, (unsigned)mmap->base_addr_high, (unsigned)mmap->base_addr_low,
             (unsigned)mmap->type, (unsigned)mmap->length_high, (unsigned)mmap->length_low);
  }
